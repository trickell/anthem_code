...
class CustomProduct extends ProposalsProductTemplate {
	...
	// 'Static File Name.df' => 'Display Name'
	private $approved_docs = array(
		'CAINT_Portal_Redesign_Flyer.pdf' => 'Portal Redesign Flyer',
		'CAINT_Pharmacy_Listing_Flyer.pdf' => 'Pharmacy Listing Flyer',
		'CAINT_Zagat_Flyer.pdf' => 'Zagat Flyer',
	);

	public $selected_options_field = 'products';

	// Commented out products are due to missing copy from client.
	// Verified with client that it is OK to remove for now.
	private $products_order = array(
		'HMO', 'PPO', 'POS', 'EPO',
		'Lumenos CDHP', //'Flexible Spending Account',
		'Dental', 'Vision', //'Stop Loss', 
		'Behavioral Health/EAP', 'Life',
		'Short/Long Term Disability',
		//'Pharmacy', 'Senior Products', '360° Health'
	);
	private $prop_type;
...
<p>Please let me know when we can discuss the details of our proposal and what other areas I can help with after your initial review.  I look forward to working with you on this opportunity!</p>";
	...
	private function setupBasicInfo() {
		...
		$page->add('proposal_type', 'Type of Case', 'radios', '', array('required'), array(
			'values' => array(
				'proposal' => 'Proposal - Non-ASO',
				'proposal_aso' => 'Proposal - ASO'
			),
			'wrap' => TRUE
		));
		...
		$this->formidable->add('show_products', FALSE, 'hidden', '');

		// put the selected products on their own page, so they aren't missed
		$page2 = $this->formidable->addWizardPage('Products Included', array(), 'show_products=true');
	...
	private function setupFinancialSummary() {
		$types = array(
			'dental' => array('Health + Dental', 1, 'modules=Dental'),
			'bvvision' => array('Health + Blue View Vision', 1, 'modules=Vision'),
			'life' => array('Health + Life ($25K or more)', 1, 'modules=Life'),
			'shortdis' => array('Health + Short Term Disability', '0.5', 'modules=Short/Long Term Disability'),
			'longdis' => array('Health + Long Term Disability', '0.5', 'modules=Short/Long Term Disability')
		);
		
		$page = $this->formidable->addWizardPage('Specialty Savings Calculator', array(), 'products=Dental,Life,Short/Long Term Disability,Vision');
	...
	private function setupPreapproved() {
		...
		$approved_docs = array();
		foreach($this->approved_docs as $file => $name) {
	...
	protected function start_proposal_generation() {
		...
		$this->prop_type = $values->proposal_type == 'proposal_aso' ? 'proposal_ASO' : $values->proposal_type;

		if(!isset($values->products)) {
			$values->products = array();
		} else if(!is_array($values->products)) {
	...
	private function generateCoverLetter($values) {
		...
		$template = 'CAINT '.ucfirst($this->prop_type).' Cover Letter.indd';
		$map = array(
		...
		if($values->cover_broker == 'Broker') {
			...
			$map['Text_12'] = $values->cover_broker_firstname.' '.$values->cover_broker_lastname.' and ';
		}
		...
		$map['Text_9'] = $values->salesrep_title;
		...
		// the InDesign file.
		$map['Text_7'] = implode('<0x000D>', $prods[0]);
		$map['Text_10'] = implode('<0x000D>', $prods[1]);
		$map['Text_11'] = implode('<0x000D>', $prods[2]);
	...
	private function generateContent($values) {
		if($this->prop_type == 'proposal_ASO') {
			return array($this->findStatic('CAINT Proposal_ASO Content.pdf') => 'ASO Proposal Content');
		} else {
			$template = 'CAINT '.ucfirst($this->prop_type).' Content.indd';
			$map = array(
				'Text_5' => 'group_name'
			);
			$idx = 10;
			foreach($this->products_order as $prod) {
				...
				$filename = DESIGNMERGE_TEMPLATE.'CA Integrated Snippets/'.ucfirst($this->prop_type).'/'.$filename;
				if(!file_exists($filename)) {
					continue;
				}
				$map['*Text_'.$idx] = file_get_contents($filename);
				$idx++;
			}
	...
	private function generateStatements($values) {
		...
		switch($values->statement_type) {
			case 'statement_1':
				return $this->build_workflow($values, '2-stmt1', 'CAINT Calculator 1.indd', $map, 'Statement 1');
			case 'statement_2':
				return $this->build_workflow($values, '2-stmt2', 'CAINT Calculator 2.indd', $map, 'Statement 2');
			default:
				return;
		}
	}

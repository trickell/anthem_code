$(function() {
	...
	var oldRegion = $("input[name=anthem_region_id]:checked").val(),
		updateRegionChange = function() {
			var val = $("input[name=anthem_region_id]:checked").val();
			if(oldRegion == val) {
				return;
			}
			oldRegion = val;
			var list = $("#approvedDocsPage input[name^=approved_docs]").attr("checked", false).each(function(i, el) {
				$(el).triggerHandler("click");
			});
		};
	$("input[name=anthem_region_id]").click(updateRegionChange);
	updateRegionChange();

	// add an only-if to the Productivity solutions option
	processOnlyIf($("#products_ProductivitySolutions")
		.attr("onlyEnableIf", "products=Short/Long Term Disability")
		.attr("uncheckOnDisable", "true")
		);
